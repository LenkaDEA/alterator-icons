...
Для соответвсвия общему стилю значков приложений альтератора иконки должны быть выполнены с учетом следущих рекомендаций:
...
где viewBox - необходим для корректного отображения необходимого разрема.
*в Figma можно использовать плагин SVG Export для корректного экспорта значков*
4. Для корректого масштабирования каждый значек должен быть реализован в 3 размерах: 16x16, 22x22, 32x32.
...
Названия значков должны соотвествовать названиям подобных значков в общих системных темах, если они заменяют их. Если же в системных темах нет подобного значка, то название можно придумать свое, отражающее то, что изображено на значке. (несколько слов в названии пишутся через "-").
...
*Это свойство можно также регулировать парамметрами svg файла, но в данном случае было принято решение использовать разные размеры по аналогии с системными темами значков.*